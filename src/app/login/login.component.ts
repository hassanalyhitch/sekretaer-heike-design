    ...
    this.localForage.listKeys().then((keys) => {
      if (keys[1]) {
        this.localForage.get(keys[1]).then((encryptedReg) => {
          let biometricRegistration = JSON.parse(
            CryptoJS.AES.decrypt(encryptedReg, environment.salt_key).toString(
              CryptoJS.enc.Utf8
            )
          );
          console.log(biometricRegistration);

          webauthn.client
            .authenticate(
              [biometricRegistration.credential.id],
            ...
            .then((authCredentials) => {
              ...
              webauthn.server
                .verifyAuthentication(authCredentials, credentialKey, expected)
                .then((valid) => {
                  // back to frontend
                  ...
                  this.localForage.get(keys[0]).then((userData) => {
                    let username = CryptoJS.AES.decrypt(
                      ...
                      environment.salt_key
                    ).toString(CryptoJS.enc.Utf8);
                    let password = CryptoJS.AES.decrypt(
                      Object.values(userData)[0],
                      environment.salt_key
                    ).toString(CryptoJS.enc.Utf8);
                    let data: LoginData = {
                      username: username,
